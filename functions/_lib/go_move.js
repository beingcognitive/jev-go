...
export function buildGoFullRequest(st, moves, me, opp, analyses, mode) {
  ...
  const criteria = { ...points, pass: mode === "assisted" ? Go.PASS_DESC(opp, st.board, me) : null };
  const questions = {
    capture_now: {
      type: "choice",
...
export function buildGoPlayerRequest(st, moves, me, opp, plan) {
  ...
  if (plan.includePass) criteria.pass = Go.PASS_DESC(opp, st.board, me);
  const questions = {
    best_move: {
      type: "choice",
      instructions:
...
export async function handleGoMove(body, env = {}) {
  try {
    ...
    const s = await openSession(env, state, "go", moves.length === 0, humanMove);
    ...
    const done = async (status, jevInfo, score = null) => ({
      ...
      body: { ok: true, game: "go", moves: mv, board: Go.toRows(st.board), captures: st.captures, toMove: st.toMove, status, score, backend: be.kind, mode, jev: jevInfo,
        state: s.verified ? await sealSession(env, "go", s, mv.length, mv) : null, gameId: s.verified ? s.id : null, verified: s.verified, owner: user && mv.length ? user.name : null },
    ...
    if (mode === "player") {
      ...
      if (plan.forced) {
        ...
        if (plan.includePass) candidates.push({ key: "pass", desc: Go.PASS_DESC(opp, st.board, me), score: 0, rank: null });
        info = {
